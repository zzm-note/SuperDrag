browser.storage.sync.get({ superDrag: _getDefault() }).then(function (result) {
    ...
    //     }, false);
    // }
    new SuperDrag(result);
    new OpenLinks(result);
                        ...
                        if (superDrag.superDrag.firstEvent) {
                            if (superDrag.superDrag.link_type[position_link] === 3) {
                                if (superDrag.superDrag.linkSearchEngines[position_link].url) {
                                    this.notice.innerHTML = this.arrow[superDrag.superDrag.direction_sel][position_link] + " 链接 - " + _link_type[superDrag.superDrag.link_type[position_link]] + " - " + browser.i18n.getMessage("custom_search");
                                } else {
                            ...
                            if (superDrag.superDrag.img_type[position_img] === 5) {
                                if (superDrag.superDrag.imgSearchEngines[position_img].url) {
                                    this.notice.innerHTML = this.arrow[superDrag.superDrag.direction_sel][position_img] + " 图片 - " + _img_type[superDrag.superDrag.img_type[position_img]] + " - " + browser.i18n.getMessage("custom_search");
                                } else {
                    ...
                    if (urlPattern.test(keyword)) {
                        ...
                        if (superDrag.superDrag.text_type[position_text] === 0) {
                            if (superDrag.superDrag.searchEngines[position_text].url) {
                                this.notice.innerHTML = this.arrow[superDrag.superDrag.direction_sel][position_text] + " 文本 - " + _text_type[superDrag.superDrag.text_type[position_text]] + " - " + browser.i18n.getMessage("custom_search");
                            } else {
    ...
    dragend(event, superDrag) {
        if (!this._dic.timeout && !this.toCancel && ["link", "none"].indexOf(this.dragEvent.dataTransfer.dropEffect) != -1) {
            if (this.handle_type == "sendMessage") {
                browser.runtime.sendMessage(this._dic);
...
class OpenLinks {
    ...
    mouseup(e, event) {
        ...
        _dic['flag'] = 'openTable';
        browser.runtime.sendMessage(_dic);
